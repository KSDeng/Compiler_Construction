...
void insertSymbol(VAR_INFO* var_info){
    if(!symbol_list_head){
        ...
        SYMBOL_NODE* q = (SYMBOL_NODE*)malloc(sizeof(SYMBOL_NODE));
        q->info = copyVarInfo(var_info);
        q->next = NULL;
        p->next = q;
    }      
}    
...
// type insertion should be dynamic in the process of analysing
/*
void insertType(TYPE_INFO* typeInfo){
    if(!type_list_head){
        TYPE_NODE* p = (TYPE_NODE*)malloc(sizeof(TYPE_NODE));
        p->next = NULL;
        strcmp(p->info->typeName, typeInfo->typeName);
        if(strcmp(typeInfo->typeName, "int") == 0 || strcmp(typeInfo->typeName, "float") == 0){
            p->typeDetail = NULL;
        }else if(strcmp(typeInfo->typeName, "array") == 0){
            p->typeDetail->array_info
*/
// Utils
/*
ARRAY_INFO* createArrayInfo(int size, VAR_INFO* ele_info){
    ...
    p->ele_info = ele_info;
    if(debug_sema){
        printf("createArrayInfo(), size %d, ele type %d\n", p->size, p->ele_info->type);
    }
    return p;
}
STRUCT_INFO* createStructInfo(int n_fields, VAR_INFO** fields){
    ...
    p->fields = fields;
    if(debug_sema){
        printf("createStructInfo(), n_fileds %d\n", p->n_fields);
        for(int i = 0; i < p->n_fields; ++i){
            printf("%s ", (p->fields)[i]->name);
        }
        printf("\n");
    }
    return p;
...
VAR_INFO* createVarInfo(char* varType){
    ...
    p->varType = (char*)malloc(strlen(varType)+1);
...
VAR_INFO* copyVarInfo(VAR_INFO* src){
    ...
    VAR_INFO* p = (VAR_INFO*)malloc(sizeof(VAR_INFO));
    strcmp(p->varType, src->varType);
    strcmp(p->varName, src->varName);
    return p;
}
/*
void copyArrayInfo(ARRAY_INFO* dest, ARRAY_INFO* src){
    if(!src){
        printf("Error in copyArrayInfo, src is NULL\n");
        return;
    }
    if(!dest){
        printf("Error in copyArrayInfo, dest is NULL\n");
        return;
    }
    dest->size = src->size;
    copyVarInfo(dest->ele_info
void copyTypeInfo(TYPE_INFO* dest, TYPE_INFO* src){
    if(!src){ dest = NULL; return; }
    if(!dest){
        printf("Error in copyTypeInfo, src is not NULL but dest is NULL\n");
        return;
    }
    strcmp(dest->typeName, src->typeName);
    if(strcmp(src->typeName, "int")==0 || strcmp(src->typeName, "float")==0){
        dest->typeDetail = NULL;
    }else {
*/
...
void Program(Node* program){
    ...
    if(!program && debug_sema) {
        printf("Program(), program node NULL\n");
        return;
    }
...
void ExtDefList(Node* extdeflist){
    ...
    if(!extdeflist && debug_sema){
        printf("ExtDefList(), extdeflist node NULL\n");
        return;
    }
...
void ExtDef(Node* extdef){
    ...
    if(!extdef && debug_sema){
        printf("ExtDef(), extdef node NULL\n");
        return;
    }
    if(strcmp(extdef->children[1]->name, "ExtDecList") == 0){   // ExtDef -> Specifier ExtDecList SEMI
        ...
        char* typeName = Specifier(extdef->children[0]);
        FUNC_INFO* funcInfo = FunDec(extdef->children[1]);
        funcInfo->returnTypeName = (char*)malloc(strlen(typeName)+1);
        strcpy(funcInfo->returnTypeName, typeName);
        CompSt(extdef->children[2]);

        char* funcId = extdef->children[1]->children[0]->propertyValue;
        // create function symbol
        VAR_INFO* funcVariableInfo = (VAR_INFO*)malloc(sizeof(VAR_INFO));
        funcVariableInfo->varType = "function";
        funcVariableInfo->varName = (char*)malloc(strlen(funcId)+1);
        strcpy(funcVariableInfo->varName, funcId);
        insertSymbol(funcVariableInfo);
        // create new type
        TYPE_INFO* funcTypeInfo = (TYPE_INFO*)malloc(sizeof(TYPE_INFO));
        funcTypeInfo->typeName = (char*)malloc(strlen(funcId)+1);
        strcpy(funcTypeInfo->typeName, funcId);
        funcTypeInfo->typeDetail->func_info = copyFuncInfo(funcInfo);
        // insert type into type list
...
void ExtDecList(Node* extdeclist){
    ...
    if(!extdeclist && debug_sema){
        printf("ExtDecList(), extdeclist node NULL\n");
        return;
    }
...
char* Specifier(Node* specifier){
    ...
    if(!specifier && debug_sema){
        printf("Specifier(), specifier node NULL\n");
        return "";
    }
...
char* StructSpecifier(Node* structspecifier){
    ...
    if(!structspecifier && debug_sema){
        printf("StructSpecifier node NULL\n");
        return "";
    }
...
char* OptTag(Node* opttag){
    ...
    if(!opttag && debug_sema){
        printf("OptTag node NULL\n");
        return "";
    }
...
char* VarDec(Node* vardec){
    if(debug_sema) printf("VarDec()\n");
    if(!vardec && debug_sema){
        printf("VarDec node NULL\n");
        return "";
    }
    if(var->n_children == 1){        // VarDec -> ID
        char* id = vardec->children[0]->propertyValue;
        if(searchHashTable(id)){
            printf("Error in VarDec(), ID \"%s\" has already been defined\n", id);
            return "";
        }
        insertIntoHashTable(id);
        if(debug_sema) printf("VarDec(), create ID \"%s\" successfully\n", id);
        return id;
    }else if(vardec->n_children == 4){  // VarDec -> VarDec LB INT RB
        // TODO: define array
        Node* int_node = vardec->children[2];
        // vardec_info->info.array_info->size = atoi(int_node->propertyValue);
        char* varName = VarDec(vardec->children[0]);
        return varName;
    }else{
        printf("VarDec(), error with unknown production\n");
        return "";
    }
}

// return list of parameters
FUNC_INFO* FunDec(Node* fundec){
    if(debug_sema) printf("FunDec()\n");
    if(!fundec && debug_sema){
        printf("FunDec node NULL\n");
        return NULL;
    }
    FUNC_INFO* funcInfo = (FUNC_INFO*)malloc(sizeof(FUNC_INFO));
    // VAR_INFO* fundec_info = createVarInfo();
    // fundec_info->type = FUNC;
    ...
    if(fundec->n_children == 4){        // FunDec -> ID LP VarList RP
        ...
        Node* paramNodePointer2 = fundec->children[2];
        int i = 0;
        // processing VarList
        while(paramNodePointer2->n_children > 0){
            Node* paramdec = paramNodePointer2->children[0];
            char* paramType = Specifier(paramdec->children[0]);
            char* paramName = VarDec(paramdec->children[1]);    // VarDec() are supposed to return variable name
            // insert symbol into symbol list
            VAR_INFO* funcParamInfo = (VAR_INFO*)malloc(sizeof(VAR_INFO));
            funcParamInfo->varType = (char*)malloc(strlen(paramType)+1);
            strcpy(funcParamInfo->varType, paramType);
            funcParamInfo->varName = (char*)malloc(strlen(paramName)+1);
            strcpy(funcParamInfo->varName, paramName);
            insertSymbol(funcParamInfo);
...
VAR_INFO** VarList(Node* varlist){
    if(debug_sema) printf("VarList()\n");
    if(!varlist && debug_sema){
        printf("VarList node NULL\n");
        return;
    }
    VAR_INFO** varlist_info = (VAR_INFO**)malloc(sizeof(VAR_INFO*));
    if(varlist->n_children == 3){   // VarList -> ParamDec COMMA VarList
        VAR_INFO* param = ParamDec(varlist->children[0]);
        VAR_INFO** otherParams = VarList(varlist->children[2]);
    }else if(varlist->n_children == 1){ // VarList -> ParamDec
        ParamDec(varlist->children[0]);
    }else{
        printf("VarList(), error with unknown production\n");
        return;
    }
}
*/
/*
VAR_INFO* ParamDec(Node* paramdec){
    if(debug_sema) printf("ParamDec()\n");
    if(!paramdec && debug_sema){
        printf("ParamDec node NULL\n");
        return;
    }
    if(paramdec->n_children == 2){  // ParamDec -> Specifier VarDec
        char* typeName = Specifier(paramdec->children[0]);
        VarDec(paramdec->children[1]);
    }else{
        printf("ParamDec(), error with unknown production\n");
        return;
    }
}
*/
void CompSt(Node* compst){
    ...
    if(!compst && debug_sema){
        printf("Compst node NULL\n");
        return;
    }
...
void StmtList(Node* stmtlist){
    ...
    if(!stmtlist && debug_sema){
        printf("StmtList node NULL\n");
        return;
    }
...
void DefList(Node* deflist){
    ...
    if(!deflist && debug_sema){
        printf("DefList node NULL\n");
        return;
    }
...
void DecList(Node* declist, char* typeName){
    ...
    if(!declist && debug_sema){
        printf("DecList node NULL\n");
        return;
    }
    ...
    if(declist->n_children == 1){       // DecList -> Dec
        ...
        if(dec->n_children == 1){           // Dec -> VarDec
            ...
            if(vardec->n_children == 1){        // VarDec -> ID
                ...
                if(nDimension == 1){        // 1-dimensional array
                    ...
                    // external array info
                    ARRY_INFO* arrInfo1 = (ARRAY_INFO*)malloc(sizeof(ARRAY_INFO));
                    arrInfo1->size = dim1;
                ...
                printf("Unknown produection, DecList() vardec\n");
                return;
            }
        }else if(dec->n_children == 3){     // Dec -> VarDec ASSIGNOP Exp

        }else{
            printf("Unknown production, DecList() dec\n");
            return;
        }
        ...
        printf("DecList(), error with unknown production\n");
        return;
    }
}
    ...
    switch(exp->n_children){
        case 1:{
            if(strcmp(exp->children[0]->name, "ID") == 0){  // Exp -> ID
                ...
                bool searchRes = searchHashTable(id);
                if(!searchRes){
                    printf("Error in Exp() case 1, \"%s\" undefined\n", id);
                    return "";
                }
